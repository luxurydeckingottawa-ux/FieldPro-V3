...
const EstimatePortalView: React.FC<EstimatePortalViewProps> = ({ 
  ...
  // Payment-choice modal (Interac e-Transfer vs Credit Card via Stripe).
  // Opens when the customer clicks Accept & Secure Your Spot. The chosen
  // method is passed through to handleAccept so the post-sign confirmation
  // shows the right instructions. E-transfer is the recommended path —
  // no processing fees, sent direct from the customer's bank.
  const [showPaymentChoice, setShowPaymentChoice] = useState(false);
  const [paymentMethod, setPaymentMethod] = useState<'etransfer' | 'card' | null>(null);
  const [etransferCopied, setEtransferCopied] = useState<string | null>(null);
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-20">
      ...
      {!isAccepted && (
        <>
          ...
          {/* visual language (hairline, eyebrow, display title, subtitle). */}
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button
                    ...
                    className="flex-1 sm:flex-none px-8 py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2 shadow-lg shadow-slate-900/20 transition-colors"
                  >
      ...
      {/* ─── Payment Choice Modal ──────────────────────────────────────────
          Opens from the sticky Accept CTA. Two paths:
            1. Interac e-Transfer (recommended) — zero processing fees,
               shows instant instructions (email + amount + reference).
            2. Credit Card — placeholder for the upcoming Stripe Connect
               integration. Marks preference; post-sign email carries the
               payment link once Stripe is wired up.
          Either choice ends by opening the existing contract signing flow,
          so the commitment sequence remains:
            choose payment method → sign contract → confirmation → pay.
      ─────────────────────────────────────────────────────────────────── */}
      {showPaymentChoice && selectedOptionId && !isAccepted && (() => {
        const total = calculateTotal();
        const deposit = Math.round(total * 0.10);
        const reference = `LD-${job.jobNumber || job.id?.slice(0, 8) || 'DEPOSIT'}`;
        const copyToClipboard = (value: string, key: string) => {
          navigator.clipboard?.writeText(value).then(
            () => {
              setEtransferCopied(key);
              setTimeout(() => setEtransferCopied(null), 2000);
            },
            ...
            <div className="relative w-full max-w-2xl bg-white rounded-[2.5rem] shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-8 sm:p-12">
                ...
                <h2 className="text-3xl sm:text-4xl font-black text-slate-900 mb-3 tracking-tight">
                  How would you like to send the deposit?
                </h2>
                <p className="text-slate-600 mb-8">
                  A <span className="font-bold text-slate-900">${deposit.toLocaleString()}</span> deposit
                  {' '}(10% of ${total.toLocaleString()}) secures your build slot. Choose the payment method that works best for you.
                </p>

                {!paymentMethod && (
                  <div className="space-y-4">
                    {/* E-Transfer — recommended */}
                    <button
                      type="button"
                      onClick={() => setPaymentMethod('etransfer')}
                      className="w-full text-left p-6 rounded-2xl border-2 border-[#D4A853] bg-gradient-to-br from-[#D4A853]/8 to-transparent hover:from-[#D4A853]/15 transition-all group"
                    >
                      <div className="flex items-start gap-4">
                        <div className="w-14 h-14 rounded-2xl bg-[#D4A853] flex items-center justify-center shrink-0 shadow-md shadow-amber-900/20">
                          <Mail className="w-7 h-7 text-slate-900" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1 flex-wrap">
                            <h3 className="text-lg font-black text-slate-900">Interac e-Transfer</h3>
                            <span className="text-[10px] font-bold uppercase tracking-widest bg-[#D4A853] text-slate-900 px-2 py-0.5 rounded-full">
                              Recommended
                            </span>
                          </div>
                          <p className="text-sm text-slate-600 mb-2">
                            Send directly from your bank. Zero processing fees. Takes 2 minutes.
                          </p>
                          <p className="text-xs text-slate-500">
                            Instructions shown next — email, amount, reference code.
                          </p>
                        </div>
                        <ArrowRight className="w-5 h-5 text-slate-400 group-hover:text-slate-900 group-hover:translate-x-1 transition-all shrink-0 mt-2" />
                      </div>
                    </button>
                    ...
                    <button
                      ...
                      className="w-full text-left p-6 rounded-2xl border-2 border-slate-200 hover:border-slate-400 bg-white transition-all group"
                    >
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="text-lg font-black text-slate-900 mb-1">Credit Card</h3>
                          <p className="text-sm text-slate-600 mb-2">
                            Visa, Mastercard, or American Express. Secure payment link sent to your email.
                          </p>
                          <p className="text-xs text-slate-500">
                            Processed by Stripe. Small convenience fee may apply.
                          </p>
                        </div>
                        <ArrowRight className="w-5 h-5 text-slate-400 group-hover:text-slate-900 group-hover:translate-x-1 transition-all shrink-0 mt-2" />
                      </div>
                    ...
                    <p className="text-xs text-slate-500 text-center pt-2">
                      You'll sign the contract after choosing your payment method.
                    </p>
                  </div>
                )}

                {/* E-Transfer instructions panel */}
                {paymentMethod === 'etransfer' && (
                  <div className="space-y-5">
                    <button
                      type="button"
                      onClick={() => setPaymentMethod(null)}
                      className="text-xs font-bold uppercase tracking-widest text-slate-500 hover:text-slate-900 flex items-center gap-1"
                    >
                      <ArrowRight className="w-3 h-3 rotate-180" /> Back to options
                    </button>

                    <div className="rounded-2xl border-2 border-[#D4A853]/30 bg-[#D4A853]/5 p-6">
                      <h3 className="text-lg font-black text-slate-900 mb-4 flex items-center gap-2">
                        <Mail className="w-5 h-5 text-[#8a6d00]" />
                        Send Interac e-Transfer
                      </h3>

                      <div className="space-y-3">
                        {[
                          { label: 'Send to', value: COMPANY.email, key: 'email' },
                          { label: 'Amount', value: `$${deposit.toLocaleString()} CAD`, key: 'amount' },
                          { label: 'Reference / memo', value: reference, key: 'reference' },
                        ].map((row) => (
                          <div key={row.key} className="bg-white rounded-xl p-4 border border-slate-200 flex items-center justify-between gap-3">
                            <div className="min-w-0 flex-1">
                            ...
                            <button
                              ...
                              className="shrink-0 px-3 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 transition-colors flex items-center gap-1.5 text-xs font-bold text-slate-700"
                            >
                              {etransferCopied === row.key ? (
                      ...
                      <div className="mt-5 text-xs text-slate-600 space-y-1.5 leading-relaxed">
                        <p><span className="font-bold text-slate-900">1.</span> Log into your online banking and start an Interac e-Transfer.</p>
                        <p><span className="font-bold text-slate-900">2.</span> Paste the email, amount, and reference above.</p>
                        <p><span className="font-bold text-slate-900">3.</span> Send. We'll confirm by email within one business day.</p>
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      className="w-full py-4 bg-slate-900 text-white rounded-2xl font-bold text-base hover:bg-slate-800 transition-colors shadow-lg shadow-slate-900/20 flex items-center justify-center gap-2"
                    >
                ...
                {/* Credit Card / Stripe placeholder panel */}
                {paymentMethod === 'card' && (
                  <div className="space-y-5">
                    <button
                      type="button"
                    ...
                    <div className="rounded-2xl border-2 border-slate-200 bg-slate-50 p-6">
                      ...
                      <p className="text-sm text-slate-600 mb-4 leading-relaxed">
                        After you sign the contract, we'll email you a secure Stripe payment link within a few minutes. It works with Visa, Mastercard, and American Express. No credit card info is entered or stored here.
                      </p>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
            ...
            >
              ...
              <div className="p-10 sm:p-16 text-center">
                ...
                {/* Payment-method reminder — e-transfer gets full instructions repeated */}
                {/* so the customer has everything they need to complete the deposit now. */}
                {paymentMethod === 'etransfer' && (() => {
                  const total = calculateTotal();
                  const deposit = Math.round(total * 0.10);
                  const reference = `LD-${job.jobNumber || job.id?.slice(0, 8) || 'DEPOSIT'}`;
                  return (
                    <div className="rounded-2xl border-2 border-[#D4A853]/30 bg-[#D4A853]/5 p-5 mb-10 text-left">
                      <div className="flex items-center gap-2 mb-3">
                        <Mail className="w-4 h-4 text-[#8a6d00]" />
                        <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#8a6d00]">
                          Send your e-transfer
                        </p>
                      </div>
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between items-center gap-3">
                          <span className="text-slate-500 text-xs uppercase tracking-wider">To</span>
                          <span className="font-mono font-bold text-slate-900 text-right truncate">{COMPANY.email}</span>
                        </div>
                        <div className="flex justify-between items-center gap-3">
                          <span className="text-slate-500 text-xs uppercase tracking-wider">Amount</span>
                          <span className="font-mono font-bold text-slate-900">${deposit.toLocaleString()} CAD</span>
                        </div>
                        <div className="flex justify-between items-center gap-3">
                          <span className="text-slate-500 text-xs uppercase tracking-wider">Reference</span>
                ...
                {paymentMethod === 'card' && (
                  <div className="rounded-2xl border-2 border-slate-200 bg-slate-50 p-5 mb-10 text-left">
                    ...
                    <p className="text-sm text-slate-600 leading-relaxed">
                      We're preparing your secure Stripe payment link. Watch your inbox — it'll arrive within a few minutes and works with Visa, Mastercard, and Amex.
                    </p>
                  </div>
                )}

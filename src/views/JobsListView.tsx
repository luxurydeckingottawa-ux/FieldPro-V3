...
const JobsListView: React.FC<JobsListViewProps> = ({ user, jobs, onSelectJob, onViewResources }) => {
  ...
  const getStatusColor = (status: JobStatus) => {
    switch (status) {
      ...
      case JobStatus.IN_PROGRESS: return 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20';
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 sm:p-10 pb-32 transition-colors duration-300">
      <header className="mb-12">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.3em]">Live Operations</span>
          </div>
          {isEmployee && (
      ...
      <button 
        ...
        className="w-full mb-12 bg-emerald-600 rounded-[2.5rem] p-8 shadow-[0_20px_50px_rgba(5,150,105,0.2)] flex items-center justify-between group hover:bg-emerald-500 transition-all active:scale-[0.98] border border-white/10"
      >
        <div className="flex items-center gap-6">
          ...
          <div className="text-left">
            ...
            <p className="text-emerald-100 text-[10px] font-black uppercase tracking-[0.2em] opacity-70">SOPs • Build Standards • Handbooks</p>
          </div>
        </div>
        ...
        {filteredJobs.length > 0 ? (
          filteredJobs.map((job) => (
            <button
              ...
              className="w-full text-left bg-[var(--card-bg)] rounded-[2.5rem] p-8 sm:p-10 shadow-xl border border-[var(--card-border)] hover:border-emerald-500/30 transition-all active:scale-[0.98] group relative overflow-hidden"
            >
              ...
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-8 relative z-10">
                <div className="space-y-6 flex-1">
                  ...
                  <div>
                    <h3 className="text-3xl sm:text-4xl font-black text-[var(--text-primary)] uppercase tracking-tight group-hover:text-emerald-500 transition-colors leading-none italic">
                      {job.clientName}
                    </h3>
                ...
                <div className="flex flex-col items-start sm:items-end justify-between gap-6 sm:min-w-[160px]">
                  ...
                  <div className="w-full sm:w-auto">
                    <div className="flex items-center justify-center gap-3 px-8 py-4 bg-emerald-600 text-black rounded-2xl text-xs font-black uppercase tracking-[0.2em] group-hover:bg-emerald-500 transition-all shadow-xl shadow-emerald-600/10 active:scale-95">

...
const CustomersListView: React.FC<CustomersListViewProps> = ({ jobs, onOpenJob, onPreviewPortal }) => {
  ...
  const LIFECYCLE_STAGES = useMemo(() => [
    ...
    { id: CustomerLifecycle.ACTIVE_JOB, label: 'Active Jobs', color: 'text-emerald-500', bg: 'bg-emerald-500/10' },
    ...
    <AnimatePresence>
      {aiDraft && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          ...
          >
            <div className="p-6 border-b border-[var(--border-color)] flex items-center justify-between bg-emerald-500/5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest text-emerald-600">{aiDraft.type.replace(/_/g, ' ')}</p>
                </div>
              </div>
              <button 
            ...
            <div className="p-6 space-y-4">
              <div className="relative">
                <textarea 
                  ...
                  className="w-full h-48 p-4 bg-[var(--text-primary)]/5 border border-[var(--border-color)] rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/50 resize-none font-sans leading-relaxed"
                />
                <button 
                ...
                >
                  {copySuccess ? (
                    <>
                      ...
                      <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest">Copied</span>
                    </>
                  ) : (
                    <>
                      <Copy className="w-4 h-4 text-[var(--text-secondary)] group-hover:text-emerald-500" />
                      <span className="text-[10px] font-bold text-[var(--text-secondary)] group-hover:text-emerald-500 uppercase tracking-widest">Copy Draft</span>
                    </>
                  )}
                </button>
              </div>
            ...
            <div className="p-6 bg-[var(--text-primary)]/5 flex items-center justify-end gap-3">
              ...
              <button 
                ...
                className="px-6 py-2.5 bg-emerald-500 text-white rounded-xl text-sm font-bold shadow-lg shadow-emerald-500/20 hover:bg-emerald-600 transition-all flex items-center gap-2"
              >
  ...
  const AiCopilotCard = ({ customer }: { customer: any }) => {
    ...
    return (
      <div className="bg-emerald-500/[0.03] border border-emerald-500/10 rounded-2xl p-4 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <BrainCircuit className="w-4 h-4 text-emerald-500" />
            <h4 className="text-xs font-black uppercase tracking-widest text-emerald-600">AI CRM Copilot</h4>
          </div>
          <div className="flex items-center gap-1">
            <button 
              onClick={() => handleGetSummary(customer)}
              disabled={aiLoading === 'summary'}
              className="p-1.5 hover:bg-emerald-500/10 rounded-lg text-emerald-600 transition-colors disabled:opacity-50"
        ...
        {summary ? (
          <div className="bg-white/50 border border-emerald-500/5 p-3 rounded-xl">
            <p className="text-xs text-[var(--text-primary)] leading-relaxed italic">"{summary}"</p>
          </div>
        ) : (
          <button 
            onClick={() => handleGetSummary(customer)}
            className="w-full py-2 border border-dashed border-emerald-500/20 rounded-xl text-[10px] font-bold text-emerald-600 uppercase tracking-widest hover:bg-emerald-500/5 transition-all flex items-center justify-center gap-2"
          >
        ...
        <div className="grid grid-cols-2 gap-3">
          {nextStep ? (
            <div className="bg-white/50 border border-emerald-500/5 p-3 rounded-xl space-y-1">
            ...
            <button 
              ...
              className="h-full py-3 border border-dashed border-emerald-500/20 rounded-xl text-[9px] font-bold text-emerald-600 uppercase tracking-widest hover:bg-emerald-500/5 transition-all flex flex-col items-center justify-center gap-1"
            >
        ...
        <div className="flex items-center gap-2 pt-1">
          <button 
            ...
            className="flex-1 py-2 bg-emerald-500/10 text-emerald-600 rounded-xl text-[9px] font-bold uppercase tracking-widest hover:bg-emerald-500/20 transition-all flex items-center justify-center gap-1.5"
          >
  ...
  const renderCustomerCard = (customer: any, isPipelineView: boolean = false) => {
    ...
    return (
      <motion.div
        ...
        className={`bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-2xl overflow-hidden shadow-sm hover:shadow-md hover:border-emerald-500/30 transition-all group flex flex-col relative cursor-pointer ${
          isExpanded 
            ? 'col-span-1 md:col-span-2 lg:col-span-2 xl:col-span-2 row-span-2 ring-2 ring-emerald-500/20 z-20' 
            : 'z-10'
        ...
        <div className={`p-3 flex-1 flex flex-col ${isExpanded ? 'space-y-4' : 'space-y-2'}`}>
          ...
          <div className="flex justify-between items-start gap-2">
            <div className="flex items-center gap-2 min-w-0">
              <div className={`h-7 w-7 rounded-lg flex items-center justify-center border shrink-0 ${isExpanded ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-gray-50 border-gray-100'}`}>
                <User className={`w-3.5 h-3.5 ${isExpanded ? 'text-emerald-500' : 'text-gray-400'}`} />
              </div>
              <div className="min-w-0">
                <h3 className="text-xs font-bold tracking-tight text-[var(--text-primary)] truncate group-hover:text-emerald-500 transition-colors">
                  {customer.name}
                </h3>
                {customer.heat === 'hot' && (
          ...
          {!isExpanded && (
            <div className="flex items-center gap-1 pt-1 border-t border-[var(--border-color)]/50">
              <button 
                ...
                className="flex-1 py-1 bg-emerald-500/5 text-emerald-600 rounded-md text-[7px] font-black uppercase tracking-widest hover:bg-emerald-500/10 transition-all"
              >
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-xl p-3">
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <span className="text-[7px] font-bold text-emerald-600/60 uppercase">{customer.nextFollowUpDate}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 bg-emerald-500/10 rounded flex items-center justify-center text-emerald-600">
                ...
                <div className="grid grid-cols-4 gap-2 pt-2">
                  ...
                  <button 
                    ...
                    className="py-2 bg-emerald-500/10 hover:bg-emerald-500/20 text-[8px] font-black uppercase tracking-widest text-emerald-600 rounded-xl transition-all flex flex-col items-center gap-1"
                  >
        ...
        return (
          <div key={stage.id} className="flex flex-col w-[300px] shrink-0 snap-start bg-gray-50/30 rounded-3xl border border-[var(--border-color)]/50 p-2">
            ...
            <div className="flex items-center justify-between mb-4 px-2 pt-2">
              ...
              <div className={`w-1.5 h-6 rounded-full ${stage.bg.replace('/10', '/100').replace('bg-', 'bg-')}`} style={{ backgroundColor: stage.bg.includes('emerald') ? '#10b981' : stage.bg.includes('blue') ? '#3b82f6' : stage.bg.includes('cyan') ? '#06b6d4' : stage.bg.includes('amber') ? '#f59e0b' : stage.bg.includes('orange') ? '#f97316' : stage.bg.includes('rose') ? '#f43f5e' : stage.bg.includes('purple') ? '#a855f7' : '#64748b' }} />
            </div>
  ...
  const getStageColor = (stage: CustomerLifecycle) => {
    switch (stage) {
      ...
      case CustomerLifecycle.ACTIVE_JOB: return 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20';
  ...
  return (
    <div className={`p-4 md:p-8 ${viewMode === 'pipeline' ? 'max-w-full' : 'max-w-full'} space-y-10`}>
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input 
              ...
              className="pl-10 pr-4 py-2.5 bg-[var(--text-primary)]/5 border border-[var(--border-color)] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/50 w-64 transition-all"
            />
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {[
          { label: 'Pipeline Value', value: `$${(stats.pipelineValue / 1000).toFixed(1)}k`, icon: BarChart3, color: 'text-emerald-500', bg: 'bg-emerald-500/5' },
      ...
      <div className="flex items-center justify-between gap-4">
            ...
            <button 
              ...
              className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest transition-all whitespace-nowrap border ${
                activeFilter === stage 
                  ...
                  : 'bg-[var(--text-primary)]/5 text-[var(--text-secondary)] border-[var(--border-color)] hover:border-emerald-500/30'
              }`}
            >
              {stage.replace(/_/g, ' ')}
            </button>
        ...
        <div className="flex items-center bg-[var(--text-primary)]/5 p-1 rounded-xl border border-[var(--border-color)] shrink-0">
          <button 
            ...
            className={`p-2 rounded-lg transition-all ${viewMode === 'pipeline' ? 'bg-[var(--bg-primary)] text-emerald-500 shadow-sm' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'}`}
            title="Pipeline View"
          >
          ...
          <button 
            ...
            className={`p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-[var(--bg-primary)] text-emerald-500 shadow-sm' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'}`}
            title="Grid View"
          >
